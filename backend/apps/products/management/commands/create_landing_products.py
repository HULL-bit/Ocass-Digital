"""
Commande Django pour créer les produits de la page d'accueil (LandingPage.tsx).
"""
...
class Command(BaseCommand):
    help = 'Crée les produits de la page d\'accueil avec leurs entreprises correspondantes'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('📦 Création des produits de la page d\'accueil...'))
            ...
            with transaction.atomic():
                ...
                for entreprise_data in entreprises_data:
                    ...
                    if created:
                        ...
                        entreprise.save()
                        self.stdout.write(self.style.WARNING(f'  🔄 Entreprise mise à jour: {entreprise.nom}'))
                ...
                for prod_data in produits_data:
                    ...
                    if created:
                        created_count += 1
                        self.stdout.write(self.style.SUCCESS(f'  ✅ Produit créé: {produit.nom} ({entreprise.nom})'))
                    else:
                        # Mettre à jour les données si nécessaire
                        ...
                        produit.save()
                        updated_count += 1
                        self.stdout.write(self.style.WARNING(f'  🔄 Produit mis à jour: {produit.nom} ({entreprise.nom})'))
                
                self.stdout.write(self.style.SUCCESS(
                    f'\n🎉 Opération terminée: {created_count} produits créés, {updated_count} produits mis à jour.'
                ))
                self.stdout.write(self.style.SUCCESS(f'📊 Total: {created_count + updated_count} produits'))
