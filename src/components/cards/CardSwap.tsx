...
const placeNow = (el: Element, slot: ReturnType<typeof makeSlot>, skew: number) =>
  gsap.set(el, {
    x: slot.x,
    y: slot.y,
    z: slot.z,
...
const CardSwap: React.FC<CardSwapProps> = ({
  ...
  useEffect(() => {
    ...
    const swap = () => {
      ...
      tl.to(elFront, {
        y: '+=500',
        duration: (config as any).durDrop,
        ease: (config as any).ease,
      });
      ...
      rest.forEach((idx, i) => {
        ...
        tl.to(
          el,
          {
            ...
            y: slot.y,
            z: slot.z,
            duration: (config as any).durMove,
            ease: (config as any).ease,
          },
          `promote+=${i * 0.15}`
        );
      ...
      tl.to(
        elFront,
        {
          ...
          y: backSlot.y,
          z: backSlot.z,
          duration: (config as any).durReturn,
          ease: (config as any).ease,
        },
        'return'
      );
